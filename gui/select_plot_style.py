...
def select_plot_style():
    root = Tk()
    root.title(" Listbox with Instructions")
    selected_values = []
    # def make_s_items(n):
    #     s_params = []
    ...
    items = ["Log Mag", "Phase", "Real", "Imag", "VSWR","Smith"]
    ...
    # Create the Listbox  selection mode
    l = Listbox(main_frame, listvariable=itemsVar, width=20, height=10) # Set width/height for better initial display
    ...
    Ctrl + Click:
    Add/remove individual Plot Style from selection.
    ...
    def show_selected_items():
        ...
        print("Selected Plot Style:", selected_values)
        if selected_values:
            messagebox.showinfo("Selected Plot Style", "\n".join(selected_values))
            
        else:
            messagebox.showinfo("Selected Plot Style", "No Plot Style selected.")
        root.destroy()
